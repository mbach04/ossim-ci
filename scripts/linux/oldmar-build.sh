...
if [ ! -f "$OSSIM_DEV_HOME/oms-$OSSIM_VERSION.jar" ]; then
   echo "ERROR: $OSSIM_DEV_HOME/oms-$OSSIM_VERSION.jar is not found in the install artifact and OMAR can't be built."
   exit 1
fi
# make sure the joms jar is in the local maven repo
mvn install:install-file -Dfile=$OSSIM_DEV_HOME/oms-$OSSIM_VERSION.jar -DgroupId=org.ossim -DartifactId=joms -Dversion=$OSSIM_VERSION -Dpackaging=jar
if [ $? -ne 0 ]; then
